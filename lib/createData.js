...
const fetchData = (apiUri, prefix = "./", config) => __awaiter(void 0, void 0, void 0, function* () {
    if (!apiUri) {
    ...
    openJsonArr.forEach(([pathStr, jsonData]) => {
        ...
        const outputFolder = path_1.default.join(config.output, prefix, pathArr.slice(0, pathArr.length - 1).join("/"));
        console.log("outputFolder: ", outputFolder);
        const filePath = path_1.default.join(config.output, prefix, `${pathStr}.json`);
        // 创建输出目录
...
const main = (config) => __awaiter(void 0, void 0, void 0, function* () {
    for (const key in config.serviceMap) {
        const apiUri = config.serviceMap[key];
        yield fetchData(apiUri, config.serviceNameToPath ? key : "./", config);
    }
});
