...
export interface IJSONResult分页信息动态拓展数据VO员工是否允许登录DTO {
    ...
    msg: string;
    // 响应结果
    data: I分页信息动态拓展数据VO员工是否允许登录DTO;
    // 服务器结果返回时的 Unix timestamp,单位毫秒
    ts: number;
}
// 分页信息«动态拓展数据VO«员工是否允许登录DTO»»
export interface I分页信息动态拓展数据VO员工是否允许登录DTO {
    // 当前页码
    pageNo: number;
    // 分页大小
    pageSize: number;
...
export interface I动态拓展数据VO员工是否允许登录DTO {
    // 拓展数据
    ext: I员工是否允许登录DTO;
    // 动态数据
    data;
