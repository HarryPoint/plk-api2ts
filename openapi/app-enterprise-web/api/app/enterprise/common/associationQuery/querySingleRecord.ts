...
// http://47.108.139.107:16400/doc.html#/default/公共相关/queryAssociationFormSingleRecordUsingPOST
export default function fetchMethod(data: IPaging1) {
    return post<IJSONResultAssociationFormTypeFieldSingleRecordOutputVO['data']>({
      url: "/app-enterprise-web/api/app/enterprise/common/associationQuery/querySingleRecord",
      data,
    });
}
// 分页_1
export interface IPaging1 {
    // 当前表单数据
    currentFormData: Record<string, Record<string, any>>;
    // 当前页面
    pageNo: number;
    // 分页大小
    pageSize: number;
...
export interface IJSONResultAssociationFormTypeFieldSingleRecordOutputVO {
    ...
    msg: string;
    // 响应结果
    data: IAssociationFormTypeFieldSingleRecordOutputVO;
    // 服务器结果返回时的 Unix timestamp,单位毫秒
    ts: number;
}
// AssociationFormTypeFieldSingleRecordOutputVO
export interface IAssociationFormTypeFieldSingleRecordOutputVO {
    // tableColumn列表
    formFieldList: IFormStructureVO[];
    // 关联表单数据
    associationFormData: Record<string, Record<string, any>>;
}
// 表单结构VO
